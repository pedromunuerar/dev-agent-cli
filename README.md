...
```bash
git clone https://github.com/pedromunuerar/clipflow
cd clipflow
./install.sh
```
...
```bash
clipflow-context ruta/al/archivo.js ruta/a/carpeta/
```
El contenido formateado se copiará a tu portapapeles. Pégalo de vuelta en el chat.

### 4. Revisión y Fusión Segura
Una vez que la IA te devuelva el bloque único de código pegalo en la pestaña correspondiente:
